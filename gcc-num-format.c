 ...
 * gcc-num-format.c
 *
 * Demonstrates how to output a floating point number using the same format 
 * as that adopted on earlier HP calculators (though the exponent is shown 
 * with an 'e'. Where the display width is wide enought to accomodate the
 * required number of digits the value displayed is rounded, however if the
 * display width is not wide enough to accomodate the required number of 
 * decimal places the number is truncated without being rounded.
 *
 * Compile with 'gcc -o x11-calc-register  x11-calc-register.c -lm'
 *
 ...
 * 27 Jul 13         - Finally got both sci() and eng() to work properly! - MEJT
...
int main(int argc, char *argv[]) {
   ...
   const i_limit = i_min(sizeof(d_testcase)/sizeof(d_testcase[0]), LIMIT);
...
char* s_mant(char* s_string, double d_number, int i_width) {
   ...
   case 1:
      ...
      /* debug("%15.12g %2d", d_number, i_exponent); */
   }
